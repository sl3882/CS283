...
int setup_buff(char *buff, char *user_str, int len) {
    ...
    while (*src != '\0' && buffer_index < len) {
        ...
        } else {                      // Handle non-whitespace characters
            *dst = *src;
            dst++;
            buffer_index++;
            in_whitespace = false;
...
void print_buff(char *buff, int len) {
    ...
    for (int i = 0; i < len; i++) {    // Iterate through buffer
...
int count_words(char *buff, int len, int str_len) {
    if (!buff || len <= 0 || str_len <= 0) return -1;  // Validate inputs
...
int reverse_string(char *buff, int len, int str_len) {
    if (str_len > len) return -1;  // Error if string exceeds buffer size
...
int word_print(char *buff, int len, int str_len) {
    ...
    for (int i = 0; i < str_len; i++) {
        if (*ptr == ' ' || *ptr == '.') {
            if (in_word) {         // Print word when boundary is reached
                printf("%d. ", word_num++);
                char *word_ptr = ptr - word_len;
                for (int j = 0; j < word_len; j++) putchar(*word_ptr++);
                printf(" (%d)\n", word_len);
                word_len = 0;
                in_word = 0;
            }
